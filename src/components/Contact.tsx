...
const Contact = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!endpoint) {
      toast.error('Form endpoint not configured. Set VITE_FORMSPREE_ENDPOINT.');
      return;
    }
    ...
    try {
      ...
      // Try to parse Formspree error payload
      let errorMessage = 'Failed to send message.';
      try {
        const data = await response.json();
        if (Array.isArray(data?.errors) && data.errors.length > 0) {
          errorMessage = data.errors.map((e: { message?: string }) => e.message).filter(Boolean).join(' ');
        }
      } catch {
        // ignore parse errors
      }
            ...
            <form onSubmit={handleSubmit} className="bg-card p-8 rounded-xl shadow-sm border border-border/50">
              <div className="grid grid-cols-1 gap-6">
                ...
                <button
                  ...
                  className={`bg-primary text-primary-foreground rounded-md px-6 py-3 font-medium transition-opacity ${isSubmitting ? 'opacity-70 cursor-not-allowed' : 'hover:opacity-90'}`}
                >
